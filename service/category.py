...
from core.cache import get_cached_embedding

def compute_similarity(image_features: torch.Tensor, text_features: torch.Tensor, topk: int):
    similarity = image_features @ text_features.T  # [B, T]
    topk_scores, topk_indices = similarity.topk(topk, dim=1)
    return topk_scores, topk_indices

def filter_tags_by_threshold(image_names, topk_scores, topk_indices, categories, threshold):
    results = []
    tag_counter = Counter()
...
def categorize_images(
    ...
    topk_scores, topk_indices = compute_similarity(image_features, text_features, topk)
    filtered_results, tag_counter = filter_tags_by_threshold(
        image_names, topk_scores, topk_indices, categories, similarity_threshold
    )
...
if __name__ == "__main__":
    ...
    categorize_images(image_features.cpu(), image_names, text_features.cpu(), categories)
    print('done')